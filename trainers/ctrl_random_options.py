...
class StrategicRandOpt(BaseTrain):
    def __init__(self, worker_id, config, sess, model, data, logger):
        super(StrategicRandOpt, self).__init__(sess, model, data, config, logger)
        self.config = config
    ...
    def select_action(self, strat_act_id, obs, screen, gen_features, b_queue, selection):
        ...
        action_prob_tact = np.random.choice(len(v_actions_tact))
